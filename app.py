...
@bp.route("/")
async def index():
    logging.debug("Method called: index")
...
async def favicon():
    logging.debug("Method called: favicon")
    return await bp.send_static_file("favicon.ico")


@bp.route("/assets/<path:path>")
async def assets(path):
    logging.debug(f"Method called: assets with path={path}")
...
async def openai_remote_azure_function_call(function_name, function_args):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(
            azure_functions_tool_url, data=json.dumps(body), headers=headers
        )
...
def prepare_model_args(request_body, request_headers):
    ...
    if MS_DEFENDER_ENABLED:
        ...
        )  # security component introduced here https://learn.microsoft.com/en-us/azure/defender-for-cloud/gain-end-user-context-ai
...
class AzureOpenaiFunctionCallStreamState:
    def __init__(self):
        ...
        self.current_tool_call = (
            None  # JSON with the tool name and arguments currently being streamed
        )
        self.function_messages = (
            []
        )  # All function messages to be appended to the chat history
        self.streaming_state = (
            "INITIAL"  # Streaming state (INITIAL, STREAMING, COMPLETED)
        )


async def process_function_call_stream(
    completionChunk,
    function_call_stream_state,
    request_body,
    request_headers,
    history_metadata,
    apim_request_id,
    ...
    if hasattr(completionChunk, "choices") and len(completionChunk.choices) > 0:
        ...
        if (
            ...
            and function_call_stream_state.streaming_state in ["INITIAL", "STREAMING"]
        ):
                ...
                if tool_call_chunk.id:
                    ...
                    function_call_stream_state.current_tool_call = {
                        ...
                        "tool_name": (
                            ...
                            if function_call_stream_state.tool_name == ""
                            else function_call_stream_state.tool_name
                        ),
                    }
                else:
                    function_call_stream_state.tool_arguments_stream += (
        ...
        # Function call - Streaming completed
            ...
            and function_call_stream_state.streaming_state == "STREAMING"
        ):
            function_call_stream_state.current_tool_call["tool_arguments"] = (
                function_call_stream_state.tool_arguments_stream
            )
            function_call_stream_state.tool_calls.append(
                function_call_stream_state.current_tool_call
            ...
            async for completionChunk in response:
                ...
                if stream_state == "COMPLETED":
                    request_body["messages"].extend(
                        function_call_stream_state.function_messages
                    )
                    function_response, apim_request_id = await send_chat_request(
                        request_body, request_headers
                    )
...
async def conversation():
    logging.debug("Method called: conversation")
...
def get_frontend_settings():
    logging.debug("Method called: get_frontend_settings")
    try:
...
async def add_conversation():
    logging.debug("Method called: add_conversation")
...
async def update_conversation():
    logging.debug("Method called: update_conversation")
    await cosmos_db_ready.wait()
    authenticated_user = get_authenticated_user_details(request_headers=request.headers)
    user_id = authenticated_user["user_principal_id"]
...
async def update_message():
    logging.debug("Method called: update_message")
    await cosmos_db_ready.wait()
    authenticated_user = get_authenticated_user_details(request_headers=request.headers)
    user_id = authenticated_user["user_principal_id"]
...
async def delete_conversation():
    logging.debug("Method called: delete_conversation")
    await cosmos_db_ready.wait()
...
async def list_conversations():
    logging.debug("Method called: list_conversations")
    await cosmos_db_ready.wait()
...
async def get_conversation():
    logging.debug("Method called: get_conversation")
    await cosmos_db_ready.wait()
    authenticated_user = get_authenticated_user_details(request_headers=request.headers)
    user_id = authenticated_user["user_principal_id"]
...
async def rename_conversation():
    logging.debug("Method called: rename_conversation")
    await cosmos_db_ready.wait()
    authenticated_user = get_authenticated_user_details(request_headers=request.headers)
    user_id = authenticated_user["user_principal_id"]
...
async def delete_all_conversations():
    logging.debug("Method called: delete_all_conversations")
    await cosmos_db_ready.wait()
    ## get the user id from the request headers
    authenticated_user = get_authenticated_user_details(request_headers=request.headers)
    user_id = authenticated_user["user_principal_id"]
...
async def clear_messages():
    logging.debug("Method called: clear_messages")
    await cosmos_db_ready.wait()
    ## get the user id from the request headers
    authenticated_user = get_authenticated_user_details(request_headers=request.headers)
    user_id = authenticated_user["user_principal_id"]
...
async def ensure_cosmos():
    logging.debug("Method called: ensure_cosmos")
    await cosmos_db_ready.wait()
...
async def generate_title(conversation_messages) -> str:
    logging.debug("Method called: generate_title")
